# experiments/text2sg/tests/test_extractor.py
import json
from unittest.mock import patch, MagicMock
from text2sg.extractor import parse_llm_output, build_prompt
...
from text2sg.genome import Genome, ValidationConfig
from text2sg.extractor import extract_article, verify_relations
...
# ── modo end2end (NER pass + extracción) ─────────────────────────────── #
from text2sg.extractor import extract_entities, run_extraction
import pandas as pd


def test_extract_entities_parses_ner_response():
    ner_resp = '{"actors": [{"uid": "U1", "canonical_name": "Gabriel Boric", "type": "roster_actor"}, {"uid": "U2", "canonical_name": "Evelyn Matthei", "type": "roster_actor"}]}'
    client = _FakeClient(ner_resp)
    union, tokens = extract_entities("Boric fue criticado por Matthei.", "model-x", client)
    assert "U1" in union and "U2" in union
    assert union["U1"]["canonical_names"] == ["Gabriel Boric"]
    assert union["U2"]["type"] == "roster_actor"
    assert tokens == 15


def test_extract_entities_returns_empty_on_bad_json():
    client = _FakeClient("esto no es json")
    union, tokens = extract_entities("texto", "model-x", client)
    assert union == {}


def test_build_prompt_end2end_shows_actor_list():
    g = Genome(prompt_text="Extrae relaciones.", architecture="end2end")
    union = {"U1": {"type": "roster_actor", "canonical_names": ["Boric"], "surfaces": []}}
    prompt = build_prompt(g, body="Noticia.", union=union, few_shot_examples=[])
    assert "ACTORES PRESENTES" in prompt
    assert "Boric" in prompt


def test_run_extraction_end2end_calls_ner_first():
    """En modo end2end, run_extraction hace NER pass por artículo e ignora union_map."""
    ner_resp = '{"actors": [{"uid": "U1", "canonical_name": "Boric", "type": "roster_actor"}]}'
    rel_resp = '{"entities": [], "relations": []}'
    # Dos artículos → 2 NER + 2 extracción = 4 llamadas
    client = _FakeClient(ner_resp, rel_resp, ner_resp, rel_resp)

    arts = pd.DataFrame([
        {"article_id": "A1", "body": "Texto A1"},
        {"article_id": "A2", "body": "Texto A2"},
    ])
    gold = pd.DataFrame(columns=["article_id", "u_from", "u_to", "act_type",
                                  "polarity", "issue", "evidence_quote"])
    # union_map vacío — end2end lo debe ignorar
    union_map = {}

    g = Genome(prompt_text="Extrae relaciones.", architecture="end2end")
    preds, total_tokens = run_extraction(["A1", "A2"], arts, gold, union_map, g, client)

    assert len(preds) == 2
    # 4 llamadas × 15 tokens cada una
    assert total_tokens == 60


def test_extract_article_collect_trace_captures_raw_and_drops():
    """collect_trace=True guarda el crudo del LLM + qué descartó la validación (artefacto B)."""
    g = Genome(prompt_text="p",
               validation=ValidationConfig(require_evidence_substring=True,
                                           min_quote_len=0,
                                           normalize_passive_direction=False))
    ...
    res = extract_article("a1", _BODY, {}, g, [], _FakeClient(out), collect_trace=True)
    tr = res["trace"]
    assert tr["n_before_validation"] == 2 and tr["n_after_validation"] == 1
    assert tr["n_dropped"] == 1                 # la cita inexistente la tira la validación
    assert "Matthei" in tr["rel_raw"]           # el crudo del LLM queda registrado


def test_proxy_metrics_without_gold():
    from text2sg.extractor import proxy_metrics
    preds = [
        {"article_id": "a1", "relations": [{"act_type": "attacks"}, {"act_type": "endorses"}],
         "trace": {"n_dropped": 1}},
        {"article_id": "a2", "relations": [], "trace": {"n_dropped": 0}},
        {"article_id": "a3", "relations": [{"act_type": "attacks"}], "trace": {"n_dropped": 2}},
    ]
    m = proxy_metrics(preds)
    assert m["n"] == 3
    assert m["total_relations"] == 3
    assert m["pct_zero_relations"] == round(1 / 3, 3)   # a2 sin relaciones
    assert m["act_type_dist"]["attacks"] == 2
    assert m["validation_dropped_total"] == 3


def test_run_extraction_given_entities_uses_union_map():
    """En modo given_entities, run_extraction usa el union_map pre-computado."""
    rel_resp = '{"entities": [], "relations": []}'
    client = _FakeClient(rel_resp, rel_resp)

    arts = pd.DataFrame([{"article_id": "A1", "body": "Texto"}])
    gold = pd.DataFrame(columns=["article_id", "u_from", "u_to", "act_type",
                                  "polarity", "issue", "evidence_quote"])
    union_map = {"A1": {"U1": {"canonical_names": ["Boric"], "type": "roster_actor"}}}

    g = Genome(prompt_text="p", architecture="given_entities")
    preds, total_tokens = run_extraction(["A1"], arts, gold, union_map, g, client)

    # Solo 1 llamada (sin NER pass)
    assert len(preds) == 1
